...
describe Bogus::HaveReceivedMatcher do
  ...
  let(:have_received) { isolate(Bogus::HaveReceivedMatcher) }
  ...
  it "returns a readable error message for object with no shadow" do
    ...
    have_received.matches?("foo").should be_false
    have_received.failure_message_for_should.should == Bogus::HaveReceivedMatcher::NO_SHADOW
    have_received.failure_message_for_should_not.should == Bogus::HaveReceivedMatcher::NO_SHADOW
  end
  ...
  it "returns a readable error message for fakes" do
    ...
    have_received.matches?(fake)
    have_received.failure_message_for_should.should ==
